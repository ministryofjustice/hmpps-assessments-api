...
class QuestionDependencies(questionDeps: Collection<QuestionDependencyEntity>) {
    ...
    fun getDisplayType(triggerUuid: UUID, answerValue: String?): Boolean? =
        displayTypes.get(Pair(triggerUuid, answerValue))
    ...
    fun triggerDisplayStyle(triggerUuid: UUID): (String?) -> Boolean? {
